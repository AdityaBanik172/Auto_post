...
class XPoster:
    ...
    def create_post(self):
        ...
        for attempt in range(max_attempts):
            if _VERBOSE:
                print(f"[POLL] X post {post_id} - Attempt {attempt+1}/{max_attempts}")
            ...
            try:
                ...
                if s_status == 200:
                    ...
                    if status in ["sent", "delivered", "shared", "success"] and link:
                        if _VERBOSE:
                            print(f"[OK] X post {post_id} is {status}. Link: {link}")
                        return link
                    elif status in ["failed", "error"]:
                        raise Exception(f"X: Post status reported as '{status}'")
                
            except Exception as e:
                if _VERBOSE:
                    print(f"[WARN] Error during X polling: {e}")
