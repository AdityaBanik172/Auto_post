...
IMAGE_PATH = Path("test/test.jpeg")

def upload_to_imgbb(path):
    print(f"[*] Uploading {path} to ImgBB...")
...
def get_insta_channel(token):
    print("[*] Fetching Instagram channel ID...")
    query = """
...
def post_to_insta(token, channel_id, image_url):
    print(f"[*] Attempting to post to Instagram (Channel: {channel_id})...")
    mutation = """
    ...
    variables = {
        "input": {
            ...
            "text": "Diagnostic Test Post from AutoPost Script",
            "mode": "shareNow",
...
if __name__ == "__main__":
    ...
    if not IMGBB_API_KEY or not INSTA_TOKEN:
        print("[!] Error: IMGBB_API_KEY or X_INSTA_BUFFER_ACCESS_TOKEN missing in .env.")
        exit(1)

    try:
        # 1. Upload
        img_url = upload_to_imgbb(IMAGE_PATH)
        print(f"[+] Hosted URL: {img_url}")

        # 2. Get Channel
        cid, cname = get_insta_channel(INSTA_TOKEN)
        if not cid:
            print("[!] No Instagram channel found.")
            exit(1)
        print(f"[+] Found Channel: {cname} ({cid})")
        ...
        if post_res.get("__typename") == "PostActionSuccess":
            ...
            print(f"External Link: {post_res.get('post', {}).get('externalLink')}")
        else:
            print("\n[FAILURE] Buffer/Instagram rejected the post.")
            print(f"Error Message: {post_res.get('message', 'Check logs above')}")
